...
export interface PaginatedEntries<T extends any = any> {
    entries: T[];
    total: number;
    limit: number;
    offset: number;
}
export let makePaginatedEntries = <T>(): PaginatedEntries<T> => ({
    entries: [],
    total: 0,
    offset: 0,
    limit: 0,
...
export interface ApiState {
    trades: PaginatedEntries<TradeHistoryEntry>;
}
...
export type DefaultThunkAction<R = Promise<any>> = ThunkAction<R, StoreState, ExtraArgument, AnyAction>;
