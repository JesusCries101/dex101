...
import {useERC20TokenContract} from '../../../hook/ERC20TokenContract'
...
type Props = StateProps & DispatchProps & {
	isAssetBaseUnlocked: boolean;
	isAssetQuoteUnlocked: boolean;
};
...
let WalletBalance = (props: Props) => {
	const { web3State, isAssetBaseUnlocked, isAssetQuoteUnlocked } = props;
	let {unlockAsset, getAllowance} = useAssetUnlock();
	let [unlockAsset_, setUnlockAsset_] = React.useState<((assetName: string) => void) | null>(null);
	...
	React.useEffect(() => {
		if (unlockAsset && getAllowance) {
			((unlockAsset, getAllowance) => {
				setUnlockAsset_(() => (assetName: string) => {
			...
			content = (
				<>
					<LabelWrapper>
						...
						<SwapperBottomRowMiddle
						  onClick={unlockAsset_ ? ((unlockAsset = unlockAsset_) => () => unlockAsset(currencyPair.base))() : () => {}}
						  disabled={isAssetBaseUnlocked}
						>
							{isAssetBaseUnlocked ? 'Unlocked' : 'Unlock'}
						</SwapperBottomRowMiddle>
					...
					<LabelWrapper>
						...
						<SwapperBottomRowMiddle
						  onClick={unlockAsset_ ? ((unlockAsset = unlockAsset_) => () => unlockAsset(currencyPair.quote))() : () => {}}
						  disabled={isAssetQuoteUnlocked}
                        >
							{isAssetQuoteUnlocked ? 'Unlocked' : 'Unlock'}
						</SwapperBottomRowMiddle>
	...
	const walletContent = _getWalletContent();
